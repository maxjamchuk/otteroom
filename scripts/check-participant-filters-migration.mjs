...
async function bounded(command,args,{input,cap=24576,signal=abort.signal}={}) {
  return await new Promise((resolve,reject)=>{
    ...
    try {
      ...
      child.stdout.on('data',bytes=>{size+=bytes.length;if(size>cap){chunks.length=0;stop();}else if(!failed)chunks.push(bytes);});
      child.stderr.resume();child.stdin.on('error',()=>{});child.on('error',()=>finish(1));child.on('close',finish);
      signal?.addEventListener('abort',stop,{once:true});child.stdin.end(input);timer=setTimeout(stop,20000);
...
try {
  ...
  if(Object.keys(parsed).sort().join(',')!=='members,rooms'||parsed.rooms?.length!==4||parsed.members?.length!==8
    || parsed.rooms.some((row,index)=>row.id!==`f4100000-0000-4000-8000-00000000000${index+1}`
  ...
  receipt('preserved-rooms=4 members=8 filters=0 count-zero=true candidate-suppressed=true types-unchanged=true');
} catch {
