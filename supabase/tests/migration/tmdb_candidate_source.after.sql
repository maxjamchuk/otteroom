...
select pg_temp.require(not exists(select 1 from expected_rooms e join public.rooms r on r.id=(e.row->>'id')::uuid
 where ((to_jsonb(r)-'candidate_acquisition_status'-'tmdb_movie_id'-'decision_completed_count'-'candidate_progression_status'-'candidate_sequence')||jsonb_build_object('row_xmin',r.xmin::text))<>e.row)
 and (select count(*)=6 from public.rooms where candidate_acquisition_status='pending' and tmdb_movie_id is null
   and decision_completed_count=0 and candidate_progression_status='inactive' and candidate_sequence=0)
