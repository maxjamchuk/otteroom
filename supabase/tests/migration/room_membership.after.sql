...
begin
  for old in select row from legacy_expected order by row->>'id' loop
    ...
    perform pg_temp.require((to_jsonb(r)-array['creator_user_id','required_voter_count','voter_count'])
    ...
    foreach subject in array expected_users loop
      ...
      perform pg_temp.require(result=jsonb_build_object('outcome','already_member','room_id',r.id,'room_code',r.code,
        'room_state',r.state,'is_creator',subject=r.creator_user_id,'is_voter',true,'voter_count',r.voter_count,'required_voter_count',2));
      if subject=r.creator_user_id then
        ...
        perform pg_temp.require(result=jsonb_build_object('outcome','already_created','room_id',r.id,'room_code',r.code,
          'room_state',r.state,'is_creator',true,'is_voter',true,'voter_count',r.voter_count,'required_voter_count',2));
      end if;
      if r.state='waiting' or r.movie_candidate_id is not null then
        -- Twice against the actual authenticated RPC; existing non-lowest FK
        -- cannot be replaced and Waiting cannot acquire a candidate.
        for i in 1..2 loop
          set local role authenticated;
          select to_jsonb(x) into result from public.ensure_room_candidate(r.id) x;
          reset role;
          if r.state='waiting' then
            perform pg_temp.require(result=jsonb_build_object('outcome','not_ready','candidate_id',null,'title',null,'release_year',null,'poster_key',null));
          else
            perform pg_temp.require(result=jsonb_build_object('outcome','available','candidate_id','fixture-clockwork-orchard',
              'title','The Clockwork Orchard','release_year',2023,'poster_key','clockwork-orchard'));
          end if;
        end loop;
      end if;
