...
begin
  for old in select row from legacy_expected order by row->>'id' loop
    ...
    perform pg_temp.require((to_jsonb(r)-array['creator_user_id','required_voter_count','voter_count','filter_completed_count','filter_resolution_status','candidate_acquisition_status','tmdb_movie_id','decision_completed_count'])
