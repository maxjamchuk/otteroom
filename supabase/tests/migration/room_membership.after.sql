...
select pg_temp.require(to_regclass('public.participant_filters') is not null
  ...
  and not exists(select 1 from public.rooms where filter_completed_count<>0
    or filter_resolution_status<>'pending' or candidate_acquisition_status<>'pending' or tmdb_movie_id is not null));
...
begin
  for old in select row from legacy_expected order by row->>'id' loop
    ...
    perform pg_temp.require((to_jsonb(r)-array['creator_user_id','required_voter_count','voter_count','filter_completed_count','filter_resolution_status','candidate_acquisition_status','tmdb_movie_id'])
