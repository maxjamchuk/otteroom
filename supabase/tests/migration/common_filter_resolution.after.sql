...
select pg_temp.require(not exists(
  ...
  where ((to_jsonb(r)-'filter_resolution_status'-'candidate_acquisition_status'-'tmdb_movie_id'-'decision_completed_count'-'candidate_progression_status'-'candidate_sequence')||jsonb_build_object('row_xmin',r.xmin::text))<>e.row
    ...
    or r.candidate_progression_status<>'inactive' or r.candidate_sequence<>0)
