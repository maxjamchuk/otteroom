...
export type RoomProjection = { id: string; code: string; state: string; voter_count: number; required_voter_count: number };
...
export function committedRoomSnapshot(room: RoomProjection) {
  ...
  const result = spawnSync('docker', ['exec', 'supabase_db_otteroom-room-session',
    ...
    `SELECT coalesce(json_agg(json_build_object('row', row_to_json(r), 'xmin', r.xmin::text,
      'members', (SELECT coalesce(json_agg(m ORDER BY m.user_id), '[]'::json) FROM
        (SELECT * FROM public.room_members WHERE room_id=r.id ORDER BY user_id LIMIT 6) m)) ORDER BY id), '[]'::json)
  ...
  try {
    ...
    const { row, members } = snapshots[0] as { row: StoredRoom; members: Member[] };
    if (!row || row.id !== room.id || row.code !== room.code ||
      Object.keys(row).sort().join(',') !== 'code,created_at,creation_request_id,creator_user_id,id,movie_candidate_id,required_voter_count,state,updated_at,voter_count' ||
      ...
      members.filter(m => m.is_voter).length !== row.voter_count) throw new Error();
    return { row, members, xmin: snapshots[0].xmin as string };
...
export async function ownRooms(page: Page, api: PublicApi, targetId?: string, targetCode?: string): Promise<RoomProjection[]> {
  ...
  // Session access stays inside this browser context and only the five public room fields return.
  const rows: unknown = await page.evaluate(async ({ origin, publicKey, targetId, targetCode }) => {
    ...
    const response = await fetch(`${origin}/rest/v1/rooms?select=id,code,state,voter_count,required_voter_count${filter}`, {
  ...
  if (!Array.isArray(rows) || rows.length > 10 || rows.some(row => !row ||
    Object.keys(row).sort().join(',') !== 'code,id,required_voter_count,state,voter_count' || typeof row.id !== 'string' ||
...
export async function realtimeBarrier(page: Page, holdInitial = false) {
  ...
  const response = async (r: Response) => {
    const url = new URL(r.url());
    if (url.pathname !== '/rest/v1/rooms' || !url.searchParams.get('id') || url.searchParams.get('select')?.replaceAll(' ', '') !== 'id,code,state,voter_count,required_voter_count') return;
    let bytes: Buffer;
    ...
    try {
      ...
      if (!r.ok() || !Array.isArray(rows) || rows.length !== 1 ||
        Object.keys(rows[0]).sort().join(',') !== 'code,id,required_voter_count,state,voter_count' || url.searchParams.get('id') !== `eq.${rows[0].id}`) throw new Error('E2E_SAFE_FAILURE');
      stats.reads++; changed();
...
export async function createWaiting(page: Page, diagnostics: SafeDiagnostics, configuration: CreationConfiguration = twoVoters) {
  ...
  expect(response.ok() && Object.keys(request).sort().join(',') === 'p_creation_request_id,p_creator_is_voter,p_required_voter_count' &&
    ...
    rows[0].voter_count === Number(configuration.creatorIsVoter) && rows[0].required_voter_count === configuration.requiredVoterCount).toBe(true);
