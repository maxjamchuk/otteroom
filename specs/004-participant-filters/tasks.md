...
- [x] T004 Extend the Feature 004 PostgreSQL suites with deterministic pgTAP+dblink trials for two distinct first submissions, final two submissions from 1/3, forced edit-before-final, forced final-before-edit, same-voter equal/different overlaps, committed response loss/equal retry, recovery-vs-submit coherent old-or-new snapshot behavior, rollback between row/summary work and every castable post-completion attempt; require independent authenticated READ COMMITTED PIDs, explicit lock barriers, `pg_blocking_pids`/ungranted-lock proof, exact xmin/snapshots and per-session filter/room write deltas with bounded drains/cleanup.
