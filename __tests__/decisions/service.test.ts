...
const undecided = [{ outcome: 'not_decided', my_decision: null,
  ...
  decision_set_complete: false, two_voter_agreement: false }];
...
it('recovers with only room and expected candidate after bootstrap', async () => {
  await expect(getCandidateDecision(roomId, 42)).resolves.toMatchObject({ outcome: 'not_decided' });
  ...
  expect(mockRpc).toHaveBeenCalledWith('get_room_candidate_decision', {
    p_room_id: roomId, p_expected_tmdb_movie_id: 42,
  });
});
...
it.each(['yes','no'] as const)('submits typed %s without caller/member authority', async decision => {
  ...
  await expect(submitCandidateDecision(roomId, 42, decision)).resolves.toMatchObject({
    outcome: 'accepted', myDecision: decision,
  });
  expect(mockRpc).toHaveBeenCalledWith('submit_room_candidate_decision', {
    p_room_id: roomId, p_expected_tmdb_movie_id: 42, p_decision: decision,
  });
});
...
it.each(['bootstrap','transport','database','contract'])('maps %s failure to one safe error', async boundary => {
  ...
  await expect(getCandidateDecision(roomId, 42)).rejects.toEqual(new DecisionServiceError());
});
...
it.each([
  ...
  await expect(getCandidateDecision(id, movie)).rejects.toEqual(new DecisionServiceError());
...
it.each([
  ...
  mockRpc.mockResolvedValue({ data: [{ ...undecided[0], outcome, my_decision: returned,
    decision_completed_count: 1 }], error: null });
  await expect(submitCandidateDecision(roomId, 42, 'yes')).rejects.toEqual(new DecisionServiceError());
});
