...
jest.mock('../../src/lib/supabase', () => ({ getSupabase: () => ({ rpc: mockRpc, functions: { invoke: mockInvoke }, from: mockFrom }) }));
const row = { outcome: 'created', room_id: '11111111-1111-4111-8111-111111111111', room_code: 'ABCDEF0123', room_state: 'waiting', is_creator: true, is_voter: true, voter_count: 1, required_voter_count: 2, filter_completed_count: 0, filter_resolution_status: 'pending', candidate_acquisition_status: 'pending', candidate_progression_status: 'inactive', candidate_sequence: 0, decision_completed_count: 0 } as const;
const requestId = '22222222-2222-4222-8222-222222222222';
const projection = { id: row.room_id, code: row.room_code, state: 'ready', voter_count: 2, required_voter_count: 2, filter_completed_count: 1, filter_resolution_status: 'pending', candidate_acquisition_status: 'pending', candidate_progression_status: 'inactive', candidate_sequence: 0, decision_completed_count: 0 } as const;
beforeEach(() => { jest.resetAllMocks(); mockBootstrap.mockResolvedValue({ user: { id: 'private-participant' } }); mockRpc.mockResolvedValue({ data: [row], error: null }); mockInvoke.mockResolvedValue({ data: row, error: null }); });
it('forwards exactly the safe create body through the room-create Edge boundary', async () => {
  expect(await createRoom(requestId, 2, true)).toEqual(row);
  expect(mockInvoke).toHaveBeenCalledWith('room-create', { body: { creation_request_id: requestId, required_voter_count: 2, creator_is_voter: true } });
});
it('retries supplied UUID and accepts already_created', async () => {
  mockInvoke.mockResolvedValueOnce({ data: null, error: { message: 'private SQL failure' } });
  await expect(createRoom(requestId, 2, true)).rejects.toThrow(RoomServiceError);
  mockInvoke.mockResolvedValueOnce({ data: { ...row, outcome: 'already_created' }, error: null });
  expect((await createRoom(requestId, 2, true)).outcome).toBe('already_created');
  expect(mockInvoke.mock.calls).toEqual(Array(2).fill(['room-create', { body: { creation_request_id: requestId, required_voter_count: 2, creator_is_voter: true } }]));
});
it.each(['create', 'join'])('%s waits for bootstrap before RPC', async operation => {
  ...
  expect(operation === 'create' ? mockInvoke : mockRpc).toHaveBeenCalledTimes(1);
});
...
it.each(['bootstrap', 'transport', 'rpc', 'contract'])('maps %s failure generically without retry', async boundary => {
  ...
  if (boundary === 'transport') { mockRpc.mockRejectedValue(new Error('private backend failure')); mockInvoke.mockRejectedValue(new Error('private backend failure')); }
  if (boundary === 'rpc') { mockRpc.mockResolvedValue({ data: [row], error: { message: 'private backend failure' } }); mockInvoke.mockResolvedValue({ data: row, error: { message: 'private backend failure' } }); }
  if (boundary === 'contract') { mockRpc.mockResolvedValue({ data: [], error: null }); mockInvoke.mockResolvedValue({ data: [], error: null }); }
  ...
  expect(mockInvoke).toHaveBeenCalledTimes(boundary === 'bootstrap' ? 0 : 1);
  expect(mockRpc).toHaveBeenCalledTimes(boundary === 'bootstrap' ? 0 : boundary === 'transport' || boundary === 'rpc' || boundary === 'contract' ? 1 : 0);
});
...
describe('authoritative member refetch', () => {
  ...
  it('awaits the shared session and reads only the accepted immutable ID and public projection', async () => {
    ...
    expect(mockSelect).toHaveBeenCalledWith('id, code, state, voter_count, required_voter_count, filter_completed_count, filter_resolution_status, candidate_acquisition_status, candidate_progression_status, candidate_sequence, decision_completed_count');
  ...
  it.each([null, [], [projection, projection], [{ ...projection, id: requestId }],
    ...
    [{ ...projection, required_voter_count: '2' }], [{ ...projection, required_voter_count: 1 }],
    [{ ...projection, filter_resolution_status: 'unknown' }],
    [{ ...projection, filter_resolution_status: 'compatible' }],
    [{ ...projection, creator_user_id: 'private' }]])('rejects absent/invalid/cross-room rows %#', async data => {
...
it.each(['compatible', 'incompatible'] as const)('accepts exact nine-field stored %s refetch at Ready N/N', async filter_resolution_status => {
  mockFrom.mockReturnValue({ select: mockSelect });
  mockSelect.mockReturnValue({ eq: mockEq }); mockEq.mockReturnValue({ limit: mockLimit });
  const terminal = { ...projection, filter_completed_count: 2, filter_resolution_status };
  mockLimit.mockResolvedValue({ data: [terminal], error: null });
  await expect(refetchRoom(row.room_id)).resolves.toEqual(terminal);
  expect(mockSelect).toHaveBeenCalledWith('id, code, state, voter_count, required_voter_count, filter_completed_count, filter_resolution_status, candidate_acquisition_status, candidate_progression_status, candidate_sequence, decision_completed_count');
});
...
it.each([[2,true],[3,true],[2,false],[3,false]] as const)('passes target%s and explicit voter=%s without identity argument', async (target,voting) => {
  ...
  expect(mockInvoke.mock.calls).toEqual([['room-create',{body:{creation_request_id:requestId,required_voter_count:target,creator_is_voter:voting}}]]);
