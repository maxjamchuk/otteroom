...
const row = { outcome: 'created', room_id: '11111111-1111-4111-8111-111111111111', room_code: 'ABCDEF0123', room_state: 'waiting', is_creator: true, is_voter: true, voter_count: 1, required_voter_count: 2, filter_completed_count: 0, filter_resolution_status: 'pending', candidate_acquisition_status: 'pending', decision_completed_count: 0 } as const;
const requestId = '22222222-2222-4222-8222-222222222222';
const projection = { id: row.room_id, code: row.room_code, state: 'ready', voter_count: 2, required_voter_count: 2, filter_completed_count: 1, filter_resolution_status: 'pending', candidate_acquisition_status: 'pending', decision_completed_count: 0 } as const;
...
it.each([
  ...
  ...['invalid_code', 'not_found', 'full'].map(outcome => ({ outcome, room_id: null, room_code: null, room_state: null, is_creator: null, is_voter: null, voter_count: null, required_voter_count: null, filter_completed_count: null, filter_resolution_status: null, candidate_acquisition_status: null, decision_completed_count: null })),
...
describe('authoritative member refetch', () => {
  ...
  it('awaits the shared session and reads only the accepted immutable ID and public projection', async () => {
    ...
    expect(mockSelect).toHaveBeenCalledWith('id, code, state, voter_count, required_voter_count, filter_completed_count, filter_resolution_status, candidate_acquisition_status, decision_completed_count');
