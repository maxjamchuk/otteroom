...
it('serves bounded scripted TMDB protocol responses without retaining headers or query values',async()=>{
  verify(`import assert from 'node:assert/strict';import {startTmdbStub,TMDB_STUB_SCENARIOS} from './e2e/support/tmdb-stub.ts';
    ...
    const token='synthetic-controlled-token',stub=await startTmdbStub(token),api=stub.controlUrl.replace('/__control','/3');try{assert.equal(stub.providerAddress,'0.0.0.0');const edge=new URL(stub.edgeBaseUrl);assert.equal(edge.hostname,'host.docker.internal');assert.equal(edge.pathname,'/3');let response=await fetch(stub.controlUrl,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({scenario:'candidate',hold:true})});assert.equal(response.ok,true);const rejected=await fetch(api+'/discover/movie?language=en-US',{headers:{authorization:'Bearer wrong-controlled-token'}});assert.equal(rejected.status,401);const discover=fetch(api+'/discover/movie?language=en-US&include_adult=false&include_video=false&sort_by=primary_release_date.asc&primary_release_date.gte=2000-01-01&primary_release_date.lte=2010-12-31&page=1&with_genres=18',{headers:{authorization:'Bearer '+token}});for(let i=0;i<40&&(await (await fetch(stub.controlUrl)).json()).held!==1;i++)await new Promise(r=>setTimeout(r,10));response=await fetch(stub.controlUrl,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({release:true})});assert.equal(response.ok,true);assert.equal((await (await discover).json()).results.length,5);const snapshot=await (await fetch(stub.controlUrl)).json();assert.deepEqual(snapshot,{scenario:'candidate',calls:{discover:1,details:0,configuration:0,poster:0},held:1,released:true,invalid:1,provider:{requests:2,completed:2,active:0}});assert.equal(/authorization|with_genres|release_date|Bearer|synthetic/.test(JSON.stringify(snapshot)),false);}finally{await stub.close();}`);
});
