...
import { isRoomId, narrowCreateResult, narrowJoinResult, validCandidateStatus, validDecisionCount, validFilterCount, validProgression, validResolutionStatus, validVoterCounts } from './contracts';
...
export type RoomProjection = Pick<Database['public']['Tables']['rooms']['Row'], 'id' | 'code' | 'voter_count' | 'required_voter_count'|'filter_completed_count'|'filter_resolution_status'|'candidate_acquisition_status'|'candidate_progression_status'|'candidate_sequence'|'decision_completed_count'> & { state: 'waiting' | 'ready' };
...
export async function refetchRoom(roomId: string): Promise<RoomProjection> {
  try {
    await bootstrapAnonymousSession();
    const { data, error } = await getSupabase().from('rooms').select('id, code, state, voter_count, required_voter_count, filter_completed_count, filter_resolution_status, candidate_acquisition_status, candidate_progression_status, candidate_sequence, decision_completed_count').eq('id', roomId).limit(2);
    if (error || !Array.isArray(data) || data.length !== 1) throw new RoomServiceError();
    const row = data[0];
    if (!row || Object.keys(row).sort().join(',') !== 'candidate_acquisition_status,candidate_progression_status,candidate_sequence,code,decision_completed_count,filter_completed_count,filter_resolution_status,id,required_voter_count,state,voter_count' || !isRoomId(row.id) || row.id !== roomId ||
      ...
      ||!validProgression(row.candidate_progression_status,row.candidate_sequence,
        ...
        row.state,row.filter_completed_count,row.filter_resolution_status)) throw new RoomServiceError();
