...
import { isRoomId, narrowCreateResult, narrowJoinResult, validFilterCount, validVoterCounts } from './contracts';
...
export type RoomProjection = Pick<Database['public']['Tables']['rooms']['Row'], 'id' | 'code' | 'voter_count' | 'required_voter_count'|'filter_completed_count'> & { state: 'waiting' | 'ready' };
...
export async function refetchRoom(roomId: string): Promise<RoomProjection> {
  try {
    await bootstrapAnonymousSession();
    const { data, error } = await getSupabase().from('rooms').select('id, code, state, voter_count, required_voter_count, filter_completed_count').eq('id', roomId).limit(2);
    if (error || !Array.isArray(data) || data.length !== 1) throw new RoomServiceError();
    const row = data[0];
    if (!row || Object.keys(row).sort().join(',') !== 'code,filter_completed_count,id,required_voter_count,state,voter_count' || !isRoomId(row.id) || row.id !== roomId ||
      ...
      ||!validFilterCount(row.filter_completed_count,row.required_voter_count,row.state)) throw new RoomServiceError();
