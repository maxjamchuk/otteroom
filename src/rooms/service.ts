...
import { isRoomId, narrowCreateResult, narrowJoinResult, validVoterCounts } from './contracts';
...
export type RoomProjection = Pick<Database['public']['Tables']['rooms']['Row'], 'id' | 'code' | 'voter_count' | 'required_voter_count'> & { state: 'waiting' | 'ready' };
...
export async function refetchRoom(roomId: string): Promise<RoomProjection> {
  try {
    await bootstrapAnonymousSession();
    const { data, error } = await getSupabase().from('rooms').select('id, code, state, voter_count, required_voter_count').eq('id', roomId).limit(2);
    if (error || !Array.isArray(data) || data.length !== 1) throw new RoomServiceError();
    const row = data[0];
    if (!row || Object.keys(row).sort().join(',') !== 'code,id,required_voter_count,state,voter_count' || !isRoomId(row.id) || row.id !== roomId ||
      typeof row.code !== 'string' || normalizeRoomCode(row.code) !== row.code ||
      (row.state !== 'waiting' && row.state !== 'ready') ||
      !validVoterCounts(row.voter_count, row.required_voter_count, row.state)) throw new RoomServiceError();
    return { id: row.id, code: row.code, state: row.state, voter_count: row.voter_count, required_voter_count: row.required_voter_count };
  } catch { throw new RoomServiceError(); }
}
